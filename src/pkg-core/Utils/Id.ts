...
type ExtractNonCollectableTypeId<T extends NonCollectableId> =
	Split<T, CONST.PrefixSep> extends [
		...
		? TypeId
		: never
type ExtractCollectableTypeId<T extends CollectableId> =
	Split<T, CONST.PrefixSep> extends [
		...
		? TypeId
		: never
type ExtractCollectionTypeId<T extends CollectionId> =
	Split<T, CONST.PrefixSep> extends [
...
type ExtractPath<T extends AnyId> =
	Split<T, CONST.PrefixSep> extends [string, infer U extends string] ? U : never

type ExtractPrimaryPath<T extends AnyId> =
	Split<ExtractPath<T>, CONST.PathTypeSep> extends [
		infer U extends string,
		...string[]
	]
		? U
		: Split<ExtractPath<T>, CONST.PathTypeSep> extends [infer U extends string]
			? U
			: never
...
export type ExtractPrimaryPathKeys<T extends AnyId> = Split<
	ExtractPrimaryPath<T>
>

export type ExtractPrimaryKey<T extends AnyId> =
...
export type ExtractPrimaryAncestorKeys<T extends AnyId> = T extends CollectionId
...
export type ExtractKey<T extends AnyId> =
	Split<T> extends [
		...
		: Split<T> extends [
			...
			? Key
			: never
export type ExtractAncestorKeys<T extends AnyId> = T extends CollectableId
	...
	: T extends CollectableId
		...
		: Split<T> extends [
			...
			? AncestorKeys
			: Split<T> extends [RulesPackageId, TypeId.AnyPrimary, string]
				? []
				: never
export type ExtractParentCollectionKey<T extends CollectableId> =
	Split<T> extends [
		RulesPackageId,
		TypeId.Collectable,
		...
		? ParentCollectionKey
		: never
type ExtractCollectionAncestorKeys<T extends CollectableId> =
	Split<T> extends [
		RulesPackageId,
		TypeId.Collection,
		...
		? AncestorKeys
		: never
type ExtractCollectableAncestorKeys<T extends CollectableId> =
	Split<T> extends [
		RulesPackageId,
		TypeId.Collectable,
