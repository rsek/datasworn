...
exports.validateTableRollRanges = void 0;
/**
 *
 ...
 */
function rowsHaveSameText(a, b) {
    for (const key of rowTextKeys) {
        const k = key;
        if (a[k] !== b[k])
            throw new Error(`Expected string "${String(a[k])}" but got "${String(b[k])}"`);
    }
    return true;
}
...
function validateTableRollRanges(oracleRollable, noSort = false) {
    var _a;
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (min === null ||
            ...
            throw new Error(`Row (${i}) min and max must both be integers, or both be null`);
        if (min < rollMin)
            throw new Error(`Row (${i}) min is less than the minimum possible roll.`);
        if (max > rollMax)
            throw new Error(`Row (${i}) max is greater than the maximum possible roll.`);
        ...
        if (previousRow.roll.max !== min + 1)
            throw new Error(`Row (${i}) roll range is not sequential with previous row.`);
    }
    return true;
}
exports.validateTableRollRanges = validateTableRollRanges;
// TODO
